...
# Default/current sizes: d_model=128, 4 layers
case $MODEL in
  mamba2)
    SIZE_ARGS="--d_model 128 --d_state 64 --n_layers 4"
    ;;
  bi_lstm)
    SIZE_ARGS="--lstm_hidden_size 128 --lstm_layers 2"
    ;;
  transformer)
    SIZE_ARGS="--d_model 128 --tf_nhead 8 --tf_layers 4 --tf_dim_feedforward 512"
    ;;
esac
